...
TILE_SIZE = 30
PAUSE_HOTKEY = pygame.K_p
PAUSE_HOTKEY_NAME = pygame.key.name(PAUSE_HOTKEY)
YELLOW = (255,255,0)
BLUE = (0,0,255)
...
def serialize(blocks):
    data = {
        'grid': blocks.grid,
        'current_block': blocks.current_block.__dict__ if blocks.current_block else None,
        'next_block': blocks.next_block.__dict__ if blocks.next_block else None,
        'score': blocks.score
    }
    
    if blocks.current_block and hasattr(blocks.current_block, 'image'):
        surface_data = pygame.image.tostring(blocks.current_block.image, 'RGB')
        data['surface_data'] = base64.b64encode(surface_data).decode('utf-8')
    else:
        data['surface_data'] = None
...
def deserialize(data):
    bg = BlocksGroup()
    bg._reset_grid()
    bg._ignore_next_stop = False
    bg.score = data['score']
    bg.next_block = None
    
    current_block_data = data['current_block']
    if current_block_data:
        block_class = globals()[current_block_data['__class__']]
        bg.add(block_class(**current_block_data))
        
    if data['surface_data']:
        image_data = base64.b64decode(data['surface_data'])
        image_surface = pygame.image.load(BytesIO(image_data))
        set_image(bg.current_block, image_surface)
            
    next_block_data = data.get('next_block')

    if next_block_data:
        block_class = globals()[next_block_data['__class__']]
        bg.next_block = block_class(**next_block_data)        
    return bg

def save_game(filename, blocks):
    with open(filename, 'wb') as f:
        print(blocks)
        # serialize the game state
        #f.write(blocks.serialize())
...
class BlocksGroup(pygame.sprite.OrderedUpdates):
    ...
    #     return {
    #         'grid': self.grid,
    #         'current_block': self.current_block.__dict__,
    #         'next_block': self.next_block.__dict__,
    #         'score': self.score,
    #         'surface_data': self.__serialize_surface(self)
    #     }
    # def serialize(self):
    #     return {
    #         'grid': self.grid,
    ...
    #     next_block_data = data['next_block']
    #     if next_block_data:
    #         block_class = globals()[next_block_data['__class__']]
    #         bg.next_block = block_class(**next_block_data)
            
    #     bg._deserialize_surface(data['surface_data'])
...
def __serialize_surface(self):
    surface_data = None
    if self.current_block and hasattr(self.current_block, 'image'):
        surface_data = pygame.image.tostring(self.current_block.image, 'RGB')
        return surface_data

def _deserialize_surface(self, surface_data):
    if surface_data:
        image = pygame.image.fromtstring(surface_data, (10* TILE_SIZE, 20 * TILE_SIZE), 'RGB')
        self.current_block.image = image
...
class POPDialog(gui.Dialog):
    ...
    def __init__(self, value, **params):
        title = gui.Label("Saved Game")
        
        main = gui.Table(width=100, height=70)
        
        main = gui.Label("You game has been saved")
        
        btn = gui.Button("Awesome")
...
def main():
    pygame.init()
    pygame.display.set_caption("Tetris by Malibucola v0.1")
    ...
    filename = "tetris_game_state.pkl"
    if os.path.getsize(filename) > 0:
        try:
            saved_state = load_game(filename)
            blocks = BlocksGroup.deserialize(saved_state)
    ...
    while run:
        ...
        hotkey_text = font.render(
            str(PAUSE_HOTKEY_NAME), True, (255, 255, 255), bgcolor)

        draw_centered_surface(screen, pause_game_text, 300)
        draw_centered_surface(screen, hotkey_text, 340)
        if game_over:
